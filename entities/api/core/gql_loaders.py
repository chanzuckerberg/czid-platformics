...
def get_base_creator(sql_model, gql_type):
    ...
    # Which SQL columns we want to expose to the GQL endpoint
    sql_columns = ["collection_id"]
    for column in sql_model.__table__.columns:
        if column.name != "entity_id":  # entity_id is autogenerated
    ...
    create.arguments = gql_arguments
    return create
...
def get_base_updater(sql_model, gql_type):
    ...
    for sql_column in sql_columns:
        field = gql_type._type_definition.get_field(sql_column)
        if field:
            is_optional = field.name != "entity_id"
            default = None if is_optional else strawberry.UNSET
