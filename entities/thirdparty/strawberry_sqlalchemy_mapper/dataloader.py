...
class Batch(Generic[K, T]):
    ...
    def add_task(self, key: Any, future: Future, kwargs: Dict[str, Any]) -> None:
        task = LoaderTask[K, T](key, future, kwargs)
...
class DataLoader(Generic[K, T]):
    ...
    def load(self, key: K, **kwargs) -> Awaitable[T]:
        if self.cache:
        ...
        batch.add_task(key, future, kwargs)
...
async def dispatch_batch(loader: DataLoader, batch: Batch) -> None:
    ...
    kwargs = [task.kwargs for task in batch.tasks]
    if len(keys) == 0:
    ...
    try:
        values = await loader.load_fn(keys, **kwargs[0])
        values = list(values)
