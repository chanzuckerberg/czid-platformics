...
class Run(Entity):
    ...
    workflow_version_id: Mapped[uuid.UUID] = mapped_column(UUID, ForeignKey("workflow_version.entity_id"), nullable=True)
    workflow_version: Mapped[WorkflowVersion] = relationship(WorkflowVersion, back_populates="runs", foreign_keys=workflow_version_id)
    run_steps: Mapped[list[RunStep]] = relationship("RunStep", back_populates="run", uselist=True, foreign_keys="RunStep.run_id")
    run_entity_inputs: Mapped[list[RunEntityInput]] = relationship("RunEntityInput", back_populates="run", uselist=True, foreign_keys="RunEntityInput.run_id")
