...
class SequencingRead(Entity):
    ...
    sample_id: Mapped[uuid.UUID] = mapped_column(UUID, ForeignKey("sample.entity_id"), nullable=True, index=True)
    ...
    r1_file_id: Mapped[uuid.UUID] = mapped_column(UUID, ForeignKey("file.id"), nullable=True, index=True)
    r1_file: Mapped["File"] = relationship("File", foreign_keys=r1_file_id)
    r2_file_id: Mapped[uuid.UUID] = mapped_column(UUID, ForeignKey("file.id"), nullable=True, index=True)
    ...
    taxon_id: Mapped[uuid.UUID] = mapped_column(UUID, ForeignKey("taxon.entity_id"), nullable=True, index=True)
    ...
    primer_file_id: Mapped[uuid.UUID] = mapped_column(
        UUID, ForeignKey("genomic_range.entity_id"), nullable=True, index=True
    )
