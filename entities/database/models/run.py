...
class Run(Entity):
    ...
    run_steps_id: Mapped[uuid.UUID] = mapped_column(UUID, ForeignKey("run_step.entity_id"), nullable=True)
    run_steps: Mapped[RunStep] = relationship(RunStep, back_populates="run", foreign_keys=run_steps_id)
    run_entity_inputs_id: Mapped[uuid.UUID] = mapped_column(
        UUID, ForeignKey("run_entity_input.entity_id"), nullable=True
    )
    run_entity_inputs: Mapped[RunEntityInput] = relationship(
        RunEntityInput, back_populates="run", foreign_keys=run_entity_inputs_id
    )
