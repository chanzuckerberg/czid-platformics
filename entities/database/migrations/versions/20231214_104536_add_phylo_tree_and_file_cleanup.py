...
def upgrade() -> None:
    ...
    op.create_table('phylogenetic_tree',
    sa.Column('tree_id', sa.UUID(), nullable=True),
    sa.Column('format', sa.Enum('newick', 'auspice_v1', 'auspice_v2', name='phylogenetictreeformat', native_enum=False), nullable=False),
    ...
    op.add_column('file', sa.Column('upload_client', sa.Enum('browser', 'cli', 's3', 'basespace', name='fileuploadclient', native_enum=False), nullable=True))
...
def downgrade() -> None:
    ...
    op.add_column('consensus_genome', sa.Column('is_reverse_complement', sa.BOOLEAN(), autoincrement=False, nullable=False))
    # ### end Alembic commands ###
